...
class LandingPage4Fragment : Fragment() {
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        val view = inflater.inflate(R.layout.fragment_landing_page4, container, false)

        val buttonNext: AppCompatImageButton = view.findViewById(R.id.intro_btn_next)

        buttonNext.setOnClickListener {
            (activity as IntroLandingPageActivity).findViewById<ViewPager2>(R.id.viewPager).currentItem = 4
        }
